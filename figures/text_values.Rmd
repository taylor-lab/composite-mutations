...
```{r, echo=T}
round(100*(fraction_samples_with_composites / average_expected - 1),3)
```
...
#### Percetange of low TMB tumors with any composites
```{r, echo=T}
low_tmb_samples <- d_clinical$Tumor_Sample_Barcode[round(d_clinical$tmb) == 4]
low_tmb_samples_with_composite <- samples_with_composites[samples_with_composites %in% low_tmb_samples]
round(100*length(unique(low_tmb_samples_with_composite)) / length(unique(low_tmb_samples)))
```

### Paragraph 3

#### Percentage of composite-mutant cases that could be ascribed to known phenomena
...
## annotate each sample/gene combo as composite, resistance, or probable biallelic loss
...
## summarize each sample for any composite mutations and their possible mechanisms
...
## % of composite-mutant cases that could be ascribed to known phenomena
round(100*sum(sample_result$reason!='Unexplained composite mutation') / nrow(sample_result),3)
```

#### Percentage of composite-mutant cases attributable to resistance mutations
```{r, echo=T}
...
#### Percentage of composite-mutant cases attributable to putative biallelic-loss of TSGs
```{r, echo=T}
round(100*sum(sample_result$reason=='Probably biallelic-loss of TSG') / nrow(sample_result),3)
```  


#### Percentage of mutations in oncogenes and TSGs that are composite-mutants
```{r, echo=T}
...
## Percentage of oncogene-mutant samples with a composite mutation
...
round(100*n_composite_onc/n_samples_onc,3)

## tabulate fraction of tsg-mutant samples with a composite mutation
d_tsg <- d[Role=='TSG',]
...
round(100*n_composite_tsg/n_samples_tsg,3)
...
m <- glm.nb(composites ~ offset(log(samples)) + reptime + cds_length + percentage_gene_gc_content + panel_introduced + tcn, data=res)
...
## genes with significant enrichment for composite mutations 
dat_genes_enriched_for_composites <- dat[q_enriched < 0.01,c('Hugo_Symbol','Role','q_enriched'),with=F]
dat_genes_enriched_for_composites
```

#### Percentage of PIK3CA mutations that are composite mutations
```{r, echo=T}
## % of all pik3ca mutations that are composites
...
composites <- length(unique(strtrim(tbl$value[tbl$N > 1],9)))
ci <- binom.confint(composites, samples_overall, method='exact')
...
#### Genes depleted for composite mutations
```{r, echo=T}
dat_genes_depleted_for_composites <- dat[q_twosided < 0.01 & logOR < 0,c('Hugo_Symbol','Role','q_twosided','logOR'),with=F]
dat_genes_depleted_for_composites
```

#### IDH1 is depleted for composite mutations
```{r, echo=T}
...
## for each hotspot, count the numnber of mutant, singleton and composite samples
...
## number of evaluable composite mutations
nrow(d)
...
## here we will find composite mutations associated with resistance, tabulate their incidence in cis and trans, and compare this to the cis/trans frequencies for composites not including resistance mutations.
## NB: Phased inferred from LOH was used in the figure panel "Fraction of phase-able composite mutations" per gene, but not to calculate P-value for difference with non-resistance composites, as this inferrential phasing was not generally done for them.
...
## exclude genes depleted for composites as these are likely passengers
result <- result[!Hugo_Symbol %in% dat_genes_depleted_for_composites$Hugo_Symbol, ]
