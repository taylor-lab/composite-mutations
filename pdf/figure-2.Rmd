...
p_hist <- ggplot(pd3, aes(x=tt,y=mu,group=type)) +
    ...
    labs(x=NULL,y='% of cases',title='Fig 2a')
...
p1 <- get_qqplot(d) +
    ...
    labs(x=NULL,y=NULL,title='Fig 2b') 
...
p2 <- ggplot(ci,aes(x=mutation, y=mean)) +
    ...
    coord_flip() + 
    labs(x=NULL,y='% hotspots',title='Fig 2c')
...
\pagebreak
```{r, panel_d, fig.width=6.5, fig.height=2.5}
...
p <- ggplot(m, aes(x=role,y=mean)) +
    ...
    labs(x=NULL,y='% timeable composite mutations', title='Fig 2d') + 
