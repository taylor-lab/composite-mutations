...
d <- fread(here('data/rebuttal_only/data_mutations_phased_all.txt'))
...
res$prop_cis <- res$cis/(res$cis + res$trans)
res$prop_trans <- res$trans/(res$cis + res$trans)
...
d_clin <- fread(here('data/data_clinical.txt'))
Nsamples <- sum(d_clin$exclude==F)
...
gene_info$silent_rate <- 1e6*gene_info$silent_mutations/(gene_info$cds_length*Nsamples)
...
p <- ggplot(tmp,aes(x=enrichment_for_compounds,y=silent_rate)) +
    ...
    geom_signif(comparison=list(c('Not significant','q<0.01')), annotation=pval, y_position=3) +
    scale_y_continuous(limits=c(0,3.5)) + 
    theme_std(base_size=16) +
    labs(x='Compound mutations in genes',y='Synonymous mutations/Mb')
p
...
dyads <- fread(here('data/rebuttal_only/dyads_genic.bed'))
...
oncokb_res <- oncokb[,summarize_oncokb_tm(.SD),by=c('Hugo_Symbol','tm')]
oncokb_res$database <- 'OncoKB'


tmp <- merge(dd, oncokb_res, by='tm', all.x=T)
tmp[is.na(class),class:='Not yet annotated']
table.freq(tmp$class)
table(tmp$class=='Not yet annotated')


chasm <- fread('~/lab/projects/compound_mutations/data/processed_data/ifs/reviewer_response/chasm/chasm_cravat_gorelica_20200124_154730/Variant.Result.tsv',skip=11)
chasm <- chasm[,c('HUGO symbol','Protein sequence change','CHASM p-value','CHASM FDR','ClinVar'),with=F]
names(chasm) <- c('Hugo_Symbol','allele','chasm_pval','chasm_fdr','clinvar')
nc <- nchar(chasm$allele)
chasm$tm <- paste(chasm$Hugo_Symbol,substr(chasm$allele,1,nc-1))

collapse_chasm_variant <- function(chasm) {
    variants <- nrow(chasm)
    best_q <- min(chasm$chasm_fdr)
    clinvar <- sortunique(chasm$clinvar[chasm$clinvar!='' & !is.na(chasm$clinvar)])
    if(length(clinvar) > 0) { 
        clinvar <- paste(sortunique(clinvar),collapse='; ')
    } else {
        clinvar <- as.character(NA)
    }
    list(best_q=best_q, clinvar=clinvar, n_variants=variants)
}
chasm2 <- chasm[,collapse_chasm_variant(.SD),by=c('Hugo_Symbol','tm')]
chasm2[best_q < 0.1,class:='Predicted oncogenic (CHASM)']
chasm2[best_q >= 0.1,class:='Not predicted oncogenic (CHASM)']
chasm2$database <- 'CHASM'
chasm2[,c('n_variants'):=NULL]


## merge oncokb/chasm together, annotate residues with best overall evidence
annotations <- rbind(oncokb_res, chasm2,fill=T)
annotations[class %in% c('likely oncogenic','Predicted oncogenic (CHASM)','oncogenic'),effect:='Oncogenic/Predicted oncogenic']
annotations[class %nin% c('likely oncogenic','Predicted oncogenic (CHASM)','oncogenic'),effect:='Likely neutral/Inconclusive']
annotations$class <- factor(annotations$class, levels=c('oncogenic','likely oncogenic','Predicted oncogenic (CHASM)','inconclusive','likely neutral','Not predicted oncogenic (CHASM)'))

collapse_residue <- function(annotations) {
    annotations <- annotations[order(class),]
    out <- annotations[1,]
    out
}
annotations_collapsed <- annotations[,collapse_residue(.SD),by=c('Hugo_Symbol','tm')]

x <- melt(xtabs(~class + effect, data=annotations_collapsed))
x$class <- factor(x$class, levels=rev(c('oncogenic','likely oncogenic','Predicted oncogenic (CHASM)','inconclusive','likely neutral','Not predicted oncogenic (CHASM)')))
x$effect <- factor(x$effect, levels=c('Oncogenic/Predicted oncogenic','Likely neutral/Inconclusive'))
#sum(x$value[x$effect=='Likely neutral/Inconclusive'])
#sum(x$value[x$effect!='Likely neutral/Inconclusive'])

cols <- RColorBrewer::brewer.pal(6,'RdYlBu')
names(cols) <- rev(levels(x$class))

p <- ggplot(x, aes(x=effect,y=value)) +
    geom_bar(stat='identity',aes(fill=class),width=0.8) +
    scale_fill_manual(values=cols,name='Classification') + 
    #geom_text(data=tbl,aes(y=N,label=N),hjust=-0.2,vjust=0.5) +
    theme_std(base_size=14) +
    scale_y_continuous(expand=c(0,0),limits=c(0,800),breaks=seq(0,800,by=200)) +
    theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
    labs(x=NULL,y='Number of hotspot mutations')
p
...
active_tfbs_coding <- fread(here('data/rebuttal_only/tfbs_skcm_dhs_coding.txt'))
...
p0 <- ggplot(dm,aes(x=distance,y=value)) +
    ...
    scale_color_manual(values=cols,name=NULL) + 
    geom_vline(xintercept=FWHM,color='black',linetype=2,size=0.5) +
    xlim(c(-1000,1000)) + ylim(c(0,0.0001)) +
    theme_std(base_size=12) + 
    theme(legend.position='none') +
    labs(x='Distance from active-TFBS [bp]',y='Proportion of SNPs',title='')

## compare the fraction of singletons and compounds to fall within the FWHM range
d_compound <- d[N > 1,]
d_singleton <- d[N == 1,]
ddc <- tabulate(d_compound, active_tfbs_coding)
ddc$class <- 'SNPs in compounds'
ddc$prop <- ddc$N / sum(ddc$N)
...
p1 <- ggplot(ci,aes(x=type,y=mean)) +
    geom_bar(stat='identity',fill='black') +
    geom_signif(comparison=list(c('Compound','Singleton')),annotation=plabel,y_position=2) +
    scale_y_continuous(expand=c(0,0),limits=c(0,2.2)) + 
    geom_errorbar(aes(min=lower,max=mean,width=NA),size=1,color='white') +
    geom_errorbar(aes(min=mean,max=upper,width=NA),size=1,color='black') +
    theme_std(base_size=12) +
    theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
    labs(x=NULL,y='Prop of SNPs in highlighted region')
...
\newpage
# Rebuttal-only L
```{r panel_l, width=6, height=7} 

## load processed mutation data including hotspot annotations
d <- fread(here('data/data_mutations.txt'),select=c('Tumor_Sample_Barcode','Hugo_Symbol','HGVSp_Short','tm','exclude',
                                                    'putative_resistance_mutation','hotspot_class','hotspotid','Variant_Type'))
length(sortunique(d$hotspotid[d$hotspot_class %in% c('24k','24k+3d') & d$Variant_Type %in% 'SNP']))

d <- d[exclude==F & putative_resistance_mutation==F,]
d$tsb_gene <- paste(d$Tumor_Sample_Barcode, d$Hugo_Symbol)
tbl <- table.freq(d$tsb_gene)
d <- merge(d, tbl, by.x='tsb_gene', by.y='value', all.x=T)
d$compound <- d$N > 1

# get list of original SNP hotspots
original_hotspots <- sortunique(d$hotspotid[d$hotspot_class %in% c('24k','24k+3d') & d$Variant_Type %in% 'SNP'])

## get list of SNP hotspots by Hess et al.
## - subset Hess data for impact genes and get list of hotspot residues
s <- fread(here('data/rebuttal_only/hess_table_s1.txt'))
s <- s[q_LNP < 0.1,]

gene_info <- fread(here('data/gene_info.txt'))
s <- s[gene %in% gene_info$Hugo_Symbol]
s$residue <- as.integer(substr(s$protein_change,2,(nchar(s$protein_change)-1)))
s$ref <- strtrim(s$protein_change,1)
s <- s[!is.na(residue)] ## remove splice sites missing residue info
s$tm <- paste0(s$gene,' ',s$ref,s$residue)
new_hotspots <- sortunique(s$tm)

## get number of affected pts, number of compound pts, for both Chang and Hess hotspots
hess_affected <- length(sortunique(strtrim(d$Tumor_Sample_Barcode[d$tm %in% new_hotspots],9)))
hess_compound <- length(sortunique(strtrim(d$Tumor_Sample_Barcode[d$tm %in% new_hotspots & d$compound==T],9)))
chang_affected <- length(sortunique(strtrim(d$Tumor_Sample_Barcode[d$tm %in% original_hotspots],9)))
chang_compound <- length(sortunique(strtrim(d$Tumor_Sample_Barcode[d$tm %in% original_hotspots & d$compound==T],9)))

## compare the propotion of compound:affected in each group
ci <- adt(binom::binom.confint(c(chang_compound,hess_compound), c(chang_affected, hess_affected), method='exact'))
ci$hotspots <- c('Chang et al. 2017','Hess et al. 2019')
pval <- prop.test(c(chang_compound,hess_compound), c(chang_affected, hess_affected))$p.value
pval <- paste0('P=',prettyNum(pval, digits=1))
ci[,4:6] <- 100*ci[,4:6,with=F]

p <- ggplot(ci, aes(x=hotspots, y=mean)) +
    scale_y_continuous(expand=c(0,0),limits=c(0,12.5),breaks=seq(0,10,by=2.5)) +
    geom_bar(stat='identity',fill='#2C92BB') +
    geom_errorbar(aes(min=lower,max=upper),width=NA,size=1) +
    geom_signif(comparison=list(ci$hotspots),annotation=pval,y_position=11.5) +
    theme_std(base_size=16) +
    labs(x='Hotspot list',y='% of hotspot mutations in compound')
p
