...
d_clinical <- fread(here('data/data_clinical.txt'),select=c('Tumor_Sample_Barcode','exclude'))
...
The significance of enrichment of individual mutant residues to arise in compound mutations compared to the significance of compound enrichment among genes. 
...
## load data for gene and residue enrichment for compound mutations
...
p <- ggplot(dat,aes(x=nlog10q.gene.new,y=nlog10q.residue.new)) +
    ...
    geom_text_repel(data=residue_labs,aes(x=nlog10q.gene.new,y=nlog10q.residue.new,label=residue,color=sig),angle=0,hjust=0,vjust=0.5) +
    ...
    labs(x='Gene -log10(q-value)',y='Residue -log10(q-value)',title=NULL)
p
...
The incidence of TERT promoter mutations in the study cohort and the fraction arising as compound mutations. 
...
yinfo <- break_axis(info$n_overall,maxlower=50,minupper=500,lowerticksize=10,upperticksize=1000,ratio_lower_to_upper=0.75)
info$y_overall <- yinfo$newy
info$y_compound <- info$y_overall * info$n_compound/info$n_overall
info$y_singleton <- info$y_overall * info$n_singleton/info$n_overall

infoM <- melt(info[,c('mutation','y_compound','y_singleton'),with=F], id.vars=c('mutation'))
infoM[variable=='y_compound',variable:='Compound']
infoM[variable=='y_singleton',variable:='Singleton']
infoM$variable <- factor(infoM$variable, levels=c('Singleton','Compound'))
infoM$mutation <- factor(infoM$mutation, levels=rev(info$mutation))
info$label <- paste0(round(info$n_compound / info$n_overall,3)*100,'%')
labels <- info[,c('mutation','label','y_compound'),with=F]

cols <- c('#EF7A63','#70C1DF')
names(cols) <- c('Compound','Singleton')
...
p1 <- ggplot(infoM, aes(x=mutation, y=value)) +
    ...
    geom_text(data=labels,aes(label=label,x=mutation,y=y_compound),color='black',hjust=-0.2,vjust=0.5) +
    theme_std(base_size=14) +
    theme(axis.text.x=element_text(angle=0,hjust=1,vjust=0.5),legend.position='bottom') +
...
The co-occurrence and mutual exclusivity of compound mutations in the TERT promoter.

```{r, Panel C, fig.width=6,fig.height=4}
...
The degree of TERT expression induced by transient transfection of TERT promoter mutations individually or as cis compound mutants in three melanoma cell lines.
