---
title: "Compound-mutations in same cell"
...
load('/ifs/res/taylorlab/impact/facets/facets_0.5.14/manifests/impact_facets_qc_2019_09_05.Rdata')
valid_samples <- impact_facets_qc$tumor_sample[impact_facets_qc$fit_name=='default' & impact_facets_qc$facets_suite_qc==T]
...
d_mutations <- fread(here('data/data_mutations.txt'),select=c('Tumor_Sample_Barcode','exclude','Hugo_Symbol','Role','putative_resistance_mutation',
                                                              ...
                                                              'Start_Position','End_Position','ccf_Mcopies_lower'))
...
## subset phasing data for compounds we know are in the same cell
d_phased$valid <- d_phased$Tumor_Sample_Barcode %in% valid_samples & (d_phased$ccf_Mcopies_lower.1 + d_phased$ccf_Mcopies_lower.2 > 1)
d_phased <- d_phased[valid==T,]

## flag compounds we know are in same cell
get_compounds_per_sample_per_gene <- function(d) {
    if(nrow(d) > 1 & all(!is.na(d$ccf_Mcopies_lower)))  {
        x <- sort(d$ccf_Mcopies_lower,decreasing=F)[1:2]
        same_cell <- sum(x) > 1
    } else {
        same_cell <- F
    }
    list(same_cell=same_cell)
}
d_mutations[Tumor_Sample_Barcode %nin% valid_samples, ccf_Mcopies_lower:=NA]
d_mutations <- d_mutations[order(Tumor_Sample_Barcode, Hugo_Symbol),]
compounds_per_sample <- d_mutations[,get_compounds_per_sample_per_gene(.SD),by=c('Tumor_Sample_Barcode','Hugo_Symbol')]
setnames(compounds_per_sample,'same_cell','valid_compound') 
d_mutations <- merge(d_mutations, compounds_per_sample, by=c('Tumor_Sample_Barcode','Hugo_Symbol'), all.x=T)
d_mutations[is.na(valid_compound),valid_compound:=F]
setnames(d_mutations,'valid_compound','valid')
```
...
result <- d_mutations[valid==T,check_compounds_per_sample(.SD),by=Tumor_Sample_Barcode]
...
round(100*fraction_samples_with_compounds,3)
```
...
l <- readRDS('~/lab/projects/compound_mutations/data/processed_data/misc/same_cell_permutations.rds')
average_expected <- mean(l$predicted$prop)
average_expected
```
...
info <- d_mutations[valid==T,summarize_sample_gene(.SD),by=c('Tumor_Sample_Barcode','Hugo_Symbol')]
...
get_compound_rate_per_gene <- function(d) {
    ...
    ## get samples with valid compounds/singletons
    tbl <- table.freq(d$Tumor_Sample_Barcode[d$valid==T])
    samples <- tbl$value
    valid_compound_samples <- tbl$value[tbl$N > 1]
    valid_singleton_samples <- tbl$value[tbl$N == 1]

    ## get any samples with invalid compounds
    tbl <- table.freq(d$Tumor_Sample_Barcode[d$valid==F])
    samples <- tbl$value
    invalid_compound_samples <- tbl$value[tbl$N > 1]
    invalid_singleton_samples <- tbl$value[tbl$N == 1]

    ## get the overall list of samples with singletons in the gene, add to this samples with invalid compounds
    singleton_samples <- sortunique(c(valid_singleton_samples, invalid_singleton_samples, invalid_compound_samples))
    
    ## get the list of samples with valid compounds in the gene
    compound_samples <- sortunique(valid_compound_samples)
...
rpkg::write.tsv(dat,'~/lab/projects/compound_mutations/data/processed_data/figure2/gene_enrichment_qqplot_data_samecell.txt')

## genes with significant enrichment for compound mutations 
dat_genes_enriched_for_compounds <- dat[q_enriched < 0.01,c('Hugo_Symbol','Role','q_enriched'),with=F]
dat_genes_enriched_for_compounds
```

#### Percentage of PIK3CA mutations that are compound mutations
```{r, echo=T}
## % of all pik3ca mutations that are compounds
x <- d_mutations[exclude==F & Hugo_Symbol=='PIK3CA',c('Tumor_Sample_Barcode','Hugo_Symbol','valid'),with=F]
valid_samples <- x$Tumor_Sample_Barcode[x$valid==T]
tbl <- table.freq(x$Tumor_Sample_Barcode)
tbl[!value %in% valid_samples, N:=1]
...
```{r, echo=T}
dat_genes_depleted_for_compounds <- dat[q_twosided < 0.01 & logOR < 0,c('Hugo_Symbol','Role','q_twosided','logOR'),with=F]
dat_genes_depleted_for_compounds
```
...
dd$compound <- dd$id %in% compound & dd$valid==T
...
#### Tabulate phase-able compound mutations
```{r, echo=T}
d <- d_phased[exclude==F & !is.na(phase) & phase!='unknown',]

## count compound mutations with 3+ mutations of the same phase only once
d$id <- paste(d$Tumor_Sample_Barcode,d$Hugo_Symbol,d$phase)
d <- d[!duplicated(id),]

## number of evaluable compound mutations
nrow(d)

## tabulate compounds with different phasing outcomes
...
#### TSGs with 2 truncating variants are more likely in trans, Oncogenes with two missense more likely in cis
```{r, echo=T}
...
dat <- dat[Class %in% c('Oncogene (Non-truncating/Non-truncating)','TSG (Truncating/Truncating)'),]
dat

## P-value
m <- as.matrix(dat[,(2:3),with=F])
rownames(m) <- dat$Class
fisher.test(m,alternative='two.sided')$p.value
```


#### P-value for significance of 100% resistance mutation compounds in cis vs non-resistance
```{r, echo=T}

## here we will find compound mutations associated with resistance, tabulate their incidence in cis and trans, and compare this to the cis/trans frequencies for compounds not including resistance mutations.
## NB: Phased inferred from LOH was used in the figure panel "Fraction of phase-able compound mutations" per gene, but not to calculate P-value for difference with non-resistance compounds, as this inferrential phasing was not generally done for them.
...
## don't overcount compounds within each sample/gene/alleles/phase combination (redoing after the above label collapsing in last step)
...
## get final tabulation of phaseable compound mutations associated with therapeutic resistance
...
## get the number of cis/trans compound mutations not associated with resistance
d <- d_phased
d <- d[exclude==F & (putative_resistance_mutation.1==F & putative_resistance_mutation.2==F),]
...
# compare cis and trans rates between compounds associated with/without resistance
...
m$prop <- 100*m$prop
m$phase <- factor(m$phase, levels=c('trans','cis'))
m0$group <-  NULL
...
#### Percentage of cis compounds among oncogenes with greatest cis rate
...
## get the number of patients with cis or trans compounds in each gene
...
result <- result[total_phaseable >= 5,]

## exclude genes depleted for compounds as these are likely passengers
result <- result[!Hugo_Symbol %in% dat_genes_depleted_for_compounds$Hugo_Symbol, ]

## show results for oncogenes with decreasing cis proportion
result[Role=='Oncogene']

## show results for TP53
...
#### Residues enriched for compound mutations
```{r,echo=T}
    d <- fread(here('data/precalculated_residue_enrichment.txt'))
    length(unique(d$query_mutation[d$q.value.enriched < 0.01])) 
```

#### Genes with any enriched for compound mutations
```{r,echo=T}
...
#### PIK3CA residues enriched for compounds are almost always in cis, and always arise as missense mutations
```{r, echo=T}
...
setnames(res,'percentage_missense','% Missense')
...
#### Percentage of mutations including common TERT promoter variants that are compound
...
getN <- function(query.mutation, d) {
    ...
    n_compound <- length(unique(strtrim(compound_samples, 9)))
    n_singleton <- length(unique(strtrim(singleton_samples, 9)))
    n_overall <- length(unique(strtrim(tbl$value, 9)))
    list(mutation=query.mutation,n_compound=n_compound, n_singleton=n_singleton, n_overall=n_overall)
}
l <- lapply(mutations, getN, d)
l <- rbindlist(l)
info <- l[order(n_overall,decreasing=T),]
info <- info[n_overall >= 5,]
info$`Percentage as compounds` <- round(100*info$n_compound / info$n_overall,3)
